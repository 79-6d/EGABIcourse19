...
## the readr library can read in csvs as well, except this is done intelligently and fields can be defined better
X.csv_r <- readr::read_csv('tidyverse_dummy_import.csv',col_types=list(Occurrence_field = col_factor()))

## The readxl library is for reading other excel formats and WORKS WELL FOR DATETIME columns
## However, it does not have a column type option for factors (if you wanted to define a column as a factor)
X.xlsx_t <- readxl::read_xlsx('tidyverse_dummy_import.xlsx',1,col_types = c('guess','numeric','numeric','text','date'))
...
## note that we use dplyr::select(), not just select(), because there are several "select" functions
## in different packages. Using the namespace prefix makes it clear which one we are using and
## less prone to errors caused by e.g. packages being loaded in a different order
...
## This gets the mean height of characters, grouped by homeworld, and adds a count to get sample size
starwars %>%
  group_by(homeworld) %>%
...
### This is a dataset that you might come across - different devices taking measurements on the same day
sst_data <- data.frame(
...
## Gathering makes the dataframe go from wide to LONG format (Long format typically used by ggplot2)
sst_data %>%
...
## What films does the character Plo Koon appear in, ordered by rank? The final result should be a single table drawn from a list column
Plo.Koon <-starwars %>%
...
### This shows how you can use map to run functions over lists to get interesting model results for
### large arrays of data.  Very helpful for summarising data quickly
nested.mass.height %>%
  mutate(
    model = map(
